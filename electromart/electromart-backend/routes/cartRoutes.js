...
router.post('/add', async (req, res) => {
  ...
  try {
    ...
    await pool.query(
      `INSERT IGNORE INTO cart (userID, status) VALUES (?, 'open')`,
      [userID]
    );
...
router.get('/get/:userID', async (req, res) => {
  ...
  try {
    const [items] = await pool.query(
      `SELECT product.productID, product.name, product.price, cartItem.quantity
       FROM cartItem
       JOIN product ON product.productID = cartItem.productID
    ...
    try {
      ...
      if (existing.length > 0) {
        if (existing[0].quantity > 1) {
          await pool.query(
            `UPDATE cartItem SET quantity = quantity - 1 WHERE cartID = ? AND productID = ?`,
            [userID, productID]
          );
        } else {
